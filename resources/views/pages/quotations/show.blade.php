...
<div class="page-header">
    ...
    <div class="page-header__actions quotation-header-actions">
        ...
        <form method="POST" action="{{ route('quotations.convert', $quotation) }}" onsubmit="return confirm('Convertir en vente ?')">
            @csrf
            <button class="btn btn--primary">→ Convertir en vente</button>
        </form>
        @endif

        <form method="POST" action="{{ route('quotations.destroy', $quotation) }}" onsubmit="return confirm('Supprimer ?')">
            @csrf @method('DELETE')
