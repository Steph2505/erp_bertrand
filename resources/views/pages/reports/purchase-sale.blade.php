...
<div class="stat-grid" style="margin-bottom:24px;">
    <div class="stat-card">
        <div class="stat-card__info"><div class="stat-card__label">CA {{ $year }}</div><div class="stat-card__value" style="color:#12864B">{{ \App\Helpers\FormatHelper::money($totals['sales']) }}</div><div class="stat-card__trend stat-card__trend--flat">Ventes confirmées</div></div>
    ...
    <div class="stat-card">
        <div class="stat-card__info"><div class="stat-card__label">Achats {{ $year }}</div><div class="stat-card__value" style="color:#1749B3">{{ \App\Helpers\FormatHelper::money($totals['purchases']) }}</div><div class="stat-card__trend stat-card__trend--flat">Achats confirmés</div></div>
    ...
    <div class="stat-card">
        <div class="stat-card__info"><div class="stat-card__label">Marge brute {{ $year }}</div>
            <div class="stat-card__value" style="color:{{ $totals['margin']>=0?'#12864B':'#C4231A' }}">{{ \App\Helpers\FormatHelper::money($totals['margin']) }}</div>
...
<div class="table-wrapper">
    ...
    <table class="data-table">
        <thead><tr>
            <th>Mois</th>
            <th style="text-align:right;color:#12864B;">Ventes</th>
            <th style="text-align:right;color:#1749B3;">Achats</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td style="text-align:right;font-weight:600;color:{{ $m['margin']>=0?'#12864B':'#C4231A' }}">
        ...
        <tfoot>
            <tr style="border-top:2px solid #E2E8F0;background:#F8FAFC;font-weight:700;">
                <td>TOTAL</td>
                <td style="text-align:right;color:#12864B;">{{ \App\Helpers\FormatHelper::money($totals['sales']) }}</td>
                <td style="text-align:right;color:#1749B3;">{{ \App\Helpers\FormatHelper::money($totals['purchases']) }}</td>
                <td style="text-align:right;color:{{ $totals['margin']>=0?'#12864B':'#C4231A' }}">{{ \App\Helpers\FormatHelper::money($totals['margin']) }}</td>
            </tr>
        </tfoot>
    </table>
</div>
...
new Chart(document.getElementById('psChart'), {
    ...
    data: {
        ...
        datasets: [
            ...
            { label: 'Marge', data: @json(collect($monthly)->pluck('margin')), type:'line', borderColor:'#B45309', backgroundColor:'rgba(245,158,11,.1)', borderWidth:2, pointRadius:4, tension:.3, fill:false },
        ]
    },
