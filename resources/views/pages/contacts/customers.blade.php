...
@section('content')
<div x-data="{ showModal: false, editCustomer: null }">

<div class="page-header">
    <div class="page-header__title">
        <h2>Clients</h2>
        <p>{{ $customers->total() }} client(s)</p>
    </div>
...
<div class="table-wrapper">
            ...
            @forelse($customers as $customer)
                <tr>
                    <td style="font-weight:500">
                        <a href="{{ route('customers.show', $customer) }}" style="color:inherit;text-decoration:none;"
                    ...
                    <td>
                        <div class="data-table__actions">
                            <a href="{{ route('customers.show', $customer) }}" class="btn btn--ghost btn--sm btn--icon" title="Voir le détail">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M2.036 12.322a1.012 1.012 0 0 1 0-.639C3.423 7.51 7.36 4.5 12 4.5c4.638 0 8.573 3.007 9.963 7.178.07.207.07.431 0 .639C20.577 16.49 16.64 19.5 12 19.5c-4.638 0-8.573-3.007-9.963-7.178Z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/></svg>
                            </a>
                            <button @click="editCustomer = {{ $customer->toJson() }}; showModal = true"
                            ...
                            <form method="POST" action="{{ route('customers.destroy', $customer) }}" @submit.prevent="window.confirmDialog('Supprimer ce client ?', {variant:'danger', confirmLabel:'Supprimer'}).then(ok => ok && $el.submit())">
                                @csrf @method('DELETE')
    ...
    <div class="table-wrapper__footer">
        <span>{{ $customers->firstItem() ?? 0 }}–{{ $customers->lastItem() ?? 0 }} sur {{ $customers->total() }}</span>
