...
<div class="page">
    ...
    <div class="header">
        ...
        <div>
            <div class="invoice-label">BON DE COMMANDE</div>
            <div class="invoice-meta">
                N° <strong>{{ $purchase->reference }}</strong><br>
                Date : <strong>{{ \App\Helpers\FormatHelper::date($purchase->purchase_date) }}</strong><br>
                Statut : @php
                    $badgeClass = match($purchase->payment_status) { 'paid' => 'badge-green', 'partial' => 'badge-yellow', default => 'badge-red' };
                    $badgeLabel = match($purchase->payment_status) { 'paid' => 'Payé', 'partial' => 'Partiel', default => 'En attente' };
                @endphp
    ...
    <div class="parties">
        ...
        <div class="party-box">
            ...
            <div class="party-info">
                @if($purchase->supplier?->phone) Tél : {{ $purchase->supplier->phone }}<br> @endif
    ...
    <div style="background:#eff6ff;border-radius:8px;padding:12px 16px;margin-bottom:20px;font-size:12px;color:#1d4ed8;">
        <strong>Entrepôt de réception :</strong> {{ $purchase->warehouse->name }}
    </div>
    @endif
        ...
        <tbody>
            @foreach($purchase->items as $i => $item)
                <tr>
                    ...
                    <td>{{ $item->product?->display_name ?? $item->product_name ?? '—' }}</td>
    ...
    <div class="totals">
        <div class="totals-box">
            <div class="totals-row">
                ...
                <span>{{ \App\Helpers\FormatHelper::money($purchase->subtotal) }}</span>
            </div>
            @if($purchase->discount > 0)
            <div class="totals-row">
                <span>Remise</span>
                <span>-{{ \App\Helpers\FormatHelper::money($purchase->discount) }}</span>
            </div>
            @endif
            @if($purchase->tax_amount > 0)
            <div class="totals-row">
                <span>Taxes</span>
                <span>{{ \App\Helpers\FormatHelper::money($purchase->tax_amount) }}</span>
            </div>
            @endif
            <div class="totals-row total">
                <span>TOTAL</span>
                <span>{{ \App\Helpers\FormatHelper::money($purchase->total) }}</span>
            </div>
            @if($purchase->amount_paid > 0)
            <div class="totals-row" style="margin-top:6px;color:#64748B;">
                <span>Payé</span>
                <span>{{ \App\Helpers\FormatHelper::money($purchase->amount_paid) }}</span>
            </div>
            @if($purchase->amount_due > 0)
            <div class="totals-row" style="color:#ef4444;">
                <span>Reste dû</span>
                <span>{{ \App\Helpers\FormatHelper::money($purchase->amount_due) }}</span>
            </div>
            @endif
            @endif
        </div>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:40px;margin-bottom:30px;">
        <div style="text-align:center;">
            <div style="font-size:11px;color:#94A3B8;margin-bottom:40px;">Signature Acheteur</div>
            <div style="border-top:1px solid #e2e8f0;padding-top:8px;font-size:11px;color:#94A3B8;">Nom & Cachet</div>
        </div>
        <div style="text-align:center;">
            <div style="font-size:11px;color:#94A3B8;margin-bottom:40px;">Signature Fournisseur</div>
            <div style="border-top:1px solid #e2e8f0;padding-top:8px;font-size:11px;color:#94A3B8;">Nom & Cachet</div>
        </div>
    </div>
