@extends('layouts.app')
@section('title', "Paiements d'achat")
@section('breadcrumb')<a href="{{ route('reports.profit-loss') }}">Rapports</a><span class="sep">/</span><span class="current">Paiements achat</span>@endsection

@section('content')
<div x-data="{...listPage('{{ route('reports.api.purchase-payments') }}')}" x-init="filters = {date_from: '{{ $from }}', date_to: '{{ $to }}'}; fetch()">
<div class="page-header">
    <div class="page-header__title"><h2>Paiements d'achat</h2><p>Règlements effectués aux fournisseurs</p></div>
    <div class="page-header__actions">
        <div class="filter-tabs">
            <a href="{{ route('reports.purchase-payments') }}" class="filter-tabs__btn filter-tabs__btn--active">Achats</a>
            <a href="{{ route('reports.sale-payments') }}" class="filter-tabs__btn">Ventes</a>
        </div>
    </div>
</div>
...
<div class="table-wrapper">
    <div class="table-wrapper__header">
        ...
        <span style="font-size:13px;color:#64748B;">Total : <strong style="color:#C4231A;" x-text="extra.sum_amount ?? '—'">—</strong></span>
    </div>
    <div style="position:relative;">
        ...
        <table class="data-table">
            <thead><tr>
                <th>Date</th><th>Référence</th><th>Achat</th><th>Fournisseur</th><th>Mode</th><th>Compte</th><th style="text-align:right">Montant</th>
            </tr></thead>
            <tbody>
                ...
                <template x-for="(pmt, idx) in rows" :key="idx">
                    <tr>
                        ...
                        <td>
                            <template x-if="pmt.purchase_url">
                                <a :href="pmt.purchase_url" style="color:#1749B3;font-weight:500;" x-text="pmt.purchase_ref"></a>
                            </template>
                            <template x-if="!pmt.purchase_url">—</template>
                        </td>
                        <td x-text="pmt.supplier_name ?? '—'"></td>
                        ...
                        <td style="text-align:right;font-weight:600;color:#C4231A;" x-text="pmt.amount"></td>
                    </tr>
                </template>
            </tbody>
        </table>
