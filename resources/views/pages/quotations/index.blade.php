                        ...
                        <td>
                            <div class="data-table__actions">
                                ...
                                <template x-if="q.convert_url">
                                    <form :action="q.convert_url" method="POST" @submit.prevent="window.confirmDialog('Convertir ce devis en vente ?', {confirmLabel:'Convertir'}).then(ok => ok && $el.submit())">
                                        @csrf
                                        <button class="btn btn--primary btn--sm" title="Convertir en vente">→ Vente</button>
                                    </form>
                                </template>
                                <form :action="q.destroy_url" method="POST" @submit.prevent="window.confirmDialog('Supprimer ce devis ?', {variant:'danger', confirmLabel:'Supprimer'}).then(ok => ok && $el.submit())">
                                    @csrf @method('DELETE')
