                    ...
                    <td>
                        <div class="data-table__actions">
                            ...
                            <form method="POST" action="{{ route('users.toggle', $user) }}"
                                  @submit.prevent="window.confirmDialog('{{ $user->is_active ? 'Désactiver' : 'Activer' }} cet utilisateur ?', {variant:'{{ $user->is_active ? 'danger' : 'default' }}', confirmLabel:'{{ $user->is_active ? 'Désactiver' : 'Activer' }}'}).then(ok => ok && $el.submit())">
                                @csrf @method('PATCH')
                                @if($user->is_active)
                                <button class="btn btn--danger btn--sm btn--icon" title="Désactiver">
                                ...
                                @else
                                <button class="btn btn--ghost btn--sm btn--icon" title="Activer">
    ...
    <div class="modal" style="max-width:640px;width:100%;">
        ...
        <form :action="editUser ? '/users/'+editUser.id : '{{ route('users.store') }}'"
            ...
            <div class="form-grid form-grid--2" style="margin-bottom:16px;">
                ...
                <div class="form-group">
                    <label>Rôle <span class="required">*</span> <small style="color:#94A3B8;">(suggère des droits)</small></label>
                    <select name="role" class="form-select" x-model="selectedRole" @change="onRoleChange()" required>
